...
{
	std::vector<region_t> segmentation_method_inflection::perform(spectrum_state spect, std::initializer_list<parameter_t> params)
	{
		...
		for (size_t i = 0; i < spect.peaks.size(); i++)
		{
			...
			// get left guard
			for (int z = position - 1; z >= 2; z--)
			{
				double secondDerivationPrev = spect.derivation[z - 1].value - spect.derivation[z - 2].value;
				double secondDerivationCurr = spect.derivation[z].value - spect.derivation[z - 1].value;
				if ((secondDerivationPrev <= 0 && secondDerivationCurr > 0 || secondDerivationPrev > 0 && secondDerivationCurr <= 0) && spect.intensities[z] <= threshold)
				{
					leftGuard = z;
					break;
				}
			...
			// get right guard
			for (int z = position + 2; z < spect.derivation.size(); z++)
			{
				double secondDerivationPrev = spect.derivation[z - 1].value - spect.derivation[z - 2].value;
				double secondDerivationCurr = spect.derivation[z].value - spect.derivation[z - 1].value;
				if ((secondDerivationPrev <= 0 && secondDerivationCurr > 0 || secondDerivationPrev > 0 && secondDerivationCurr <= 0) && spect.intensities[z] <= threshold)
